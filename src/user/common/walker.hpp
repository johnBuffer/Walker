...
struct Walker
{
    ...
    Walker(Vec2 position)
    {
        ...
        // Pods
        float const leg_scale = 1.0f;
        addPod({position.x - 1.7f * base, position.y - leg_scale * base}); // 4
        addPod({position.x + 1.7f * base, position.y - leg_scale * base}); // 5
        addPod({position.x + 1.7f * base, position.y + leg_scale * base}); // 6
        addPod({position.x - 1.7f * base, position.y + leg_scale * base}); // 7

        // Muscles
        float const ratio_contraction = 0.2f;
        float const ratio_extension   = 0.15f;
        addMuscle(0, 4 , ratio_contraction, ratio_extension);
        addMuscle(3, 4 , ratio_contraction, ratio_extension);
        addMuscle(1, 5 , ratio_contraction, ratio_extension);
        addMuscle(2, 5 , ratio_contraction, ratio_extension);
        addMuscle(1, 6, ratio_contraction, ratio_extension);
        addMuscle(2, 6 , ratio_contraction, ratio_extension);
        addMuscle(0, 7, ratio_contraction, ratio_extension);
