...
struct Utils
{
    static void generateLine(sf::VertexArray& va, uint32_t index, sf::Vector2f point_1, sf::Vector2f point_2, float width, sf::Color color)
    {
        ...
        va[index + 0].position = point_1 + n * 0.5f * width;
        va[index + 1].position = point_2 + n * 0.5f * width;
        va[index + 2].position = point_2 - n * 0.5f * width;
        va[index + 3].position = point_1 - n * 0.5f * width;
    ...
    static void generateCircle(sf::VertexArray& va, float radius, uint32_t quality, sf::Color color)
    {
        ...
        float const da = Math::TwoPI / static_cast<float>(quality - 2);
