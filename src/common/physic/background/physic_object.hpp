...
struct PhysicObject
{
    ...
    void update(float dt)
    {
        ...
        const Vec2 new_position = position + last_update_move + (acceleration - last_update_move * 4.0f) * (dt * dt);
        last_position           = position;
        position                = new_position;
        acceleration = {0.0f, 0.0f};

        radius += (0.5f - radius) * 0.2f * dt;
        current_ratio += (color_ratio - current_ratio) * 0.2f * dt;
    }
